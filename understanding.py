"""
understanding_map.py — Phase 2: Two heatmap methods compared side by side
=========================================================================

METHOD 1: Per-Patch
  Mask one patch at a time. 64 forward passes per image.
  Result: 8x8 error grid → upsampled to 32x32.
  Fast but blocky.

METHOD 2: Sliding Window
  Mask a 2x2 block of patches, slide with stride 1.
  ~49 forward passes per image (7x7 valid positions).
  Result: smoother, more spatial heatmap.
  Slower but reveals more structure.

The comparison between the two methods is itself informative —
where they agree = robust signal. Where they disagree = artifact.
...
GRID = 8   # 8x8 patch grid (32px / 4px patch)
...
def per_patch_map(img_batch, encoder, predictor):
    """
    Mask each of 64 patches one at a time.
    Returns (B, 64) error array.
    """
    B          = img_batch.shape[0]
...
# ─────────────────────────────────────────────────────────────────────────────
# METHOD 2: SLIDING WINDOW (2×2 patch block, stride 1)
# ─────────────────────────────────────────────────────────────────────────────
def sliding_window_map(img_batch, encoder, predictor, window=2, stride=1):
    """
    Mask a 2×2 block of patches at a time, slide across the grid.
    Returns (B, H_out, W_out) error array where H_out = W_out = 7.
    Then upsampled to (B, 32, 32) for display.
    """
    B      = img_batch.shape[0]
    n_pos  = (GRID - window) // stride + 1   # = 7 for GRID=8, window=2, stride=1
    errors = np.zeros((B, n_pos, n_pos))

    with torch.no_grad():
        for row in range(n_pos):
            for col in range(n_pos):
                # Collect 2×2 block indices
                tgt_idx = []
...
pp_errors = per_patch_map(imgs, encoder, predictor)   # (B, 64)

print(f"Generating sliding window maps ({args.n_images} images × 49 passes)...")
sw_errors = sliding_window_map(imgs, encoder, predictor)  # (B, 7, 7)


# ─────────────────────────────────────────────────────────────────────────────
# VISUALIZE: 3 rows per image — original | per-patch | sliding window
# ─────────────────────────────────────────────────────────────────────────────
...
# Normalize error maps globally across all images for fair comparison
...
for i in range(N):
    ...
    # Row 2: sliding window map (smooth)
