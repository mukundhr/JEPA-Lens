...
def compute_error_maps(imgs_tensor, encoder, predictor):
    """
    imgs_tensor : (N, 3, 32, 32) normalized tensor
    Returns     : (N, 64) numpy array of per-patch prediction errors
    """
...
def blur_tensor(imgs_tensor, sigma):
    """Apply Gaussian blur to a batch of normalized tensors."""
...
def canny_patch_density(imgs_tensor):
    """
    Compute per-patch Canny edge density for each image.
    Returns (N, 64) numpy array — fraction of edge pixels per patch.
    """
    if not HAS_CV2:
        return None
    ...
    for i, img in enumerate(imgs_tensor):
        ...
        edges  = cv2.Canny(gray, 50, 150).astype(np.float32) / 255.0  # (32,32)
...
imgs_tensor, labels = next(iter(loader))   # (N, 3, 32, 32)
...
for sigma in args.blur_sigmas:
    ...
    # Per-image Spearman rank correlation between original and blurred error maps
    correlations = []
...
if HAS_CV2:
    ...
    edge_densities = canny_patch_density(imgs_tensor)  # (N, 64)

    # Per-image Pearson correlation between error and edge density
    edge_correlations = []
    for i in range(len(imgs_tensor)):
...
# ─────────────────────────────────────────────────────────────────────────────
# TEST 3: STRUCTURE_FOCUSED DIVERGENCE
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "="*60)
...
if has_structure:
    ...
    for i in range(len(imgs_tensor)):
        ...
        # Normalize both maps to [0,1] before MAD for fair comparison
...
# Violin of per-image correlations at max blur
...
if HAS_CV2:
    ...
    # Scatter: mean error vs mean edge density per patch
    ax2b = fig.add_subplot(gs[1, col])
    mean_err_per_patch  = errors_original.mean(axis=0)   # (64,)
    mean_edge_per_patch = edge_densities.mean(axis=0)    # (64,)
...
if has_structure:
    ...
    # Scatter: baseline error map vs structure_focused error map (patch-level means)
...
# ─────────────────────────────────────────────────────────────────────────────
# QUALITATIVE: side-by-side error map vs blur vs edge for 6 images
# ─────────────────────────────────────────────────────────────────────────────
...
# Precompute blurred error map at max sigma
...
for row in range(N_SHOW):
    ...
    err_delta = err_o - err_b  # where original was harder than blurred

    c = 0
    # Col 0: original + error overlay
    ...
    # Col 2: error delta (orig - blur) — where original was harder
    ...
    # Col 3 (optional): Canny edge density
    if HAS_CV2:
    ...
    # Col 4 (optional): structure_focused error
    if has_structure:
        axes[row, c].imshow(img_np, interpolation='nearest', alpha=0.4)
