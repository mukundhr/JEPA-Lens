"""
causal_test.py — The experiment that validates JEPA-Lens's core claim
======================================================================

CLAIM: "High prediction error reflects semantic importance"

If true, then:
  Masking high-error patches should hurt classification MORE than
  masking low-error patches (or random patches).

EXPERIMENT:
  For N images, generate per-patch error maps.
  Then run linear probe under 3 conditions:

  Condition A — ABLATE HIGH-ERROR patches (remove what model found hard)
  Condition B — ABLATE LOW-ERROR patches  (remove what model found easy)
  Condition C — ABLATE RANDOM patches     (control)

  If A degrades accuracy more than B → error map tracks semantic content.
  If A ≈ B ≈ C → error map is noise.

This converts a visual intuition into a falsifiable, quantifiable claim.
...
# Use subset for error map generation (speed), full test set for evaluation
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 1: Generate per-patch error maps for all probe images
# ─────────────────────────────────────────────────────────────────────────────
def compute_error_maps(loader, encoder, predictor, device, desc=""):
    """
    Returns:
      all_imgs   : (N, 3, 32, 32) tensor
      all_errors : (N, 64) numpy array — prediction error per patch per image
    ...
    with torch.no_grad():
        for batch_i, (imgs, lbls) in enumerate(loader):
            ...
            for tgt_i in range(NUM_PATCHES):
                ...
                err = ((pred - tgt) ** 2).mean(dim=-1).squeeze(-1)  # (B,)
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 2: Ablation function
# ─────────────────────────────────────────────────────────────────────────────
def get_ablated_representations(imgs_tensor, errors, encoder, device, k, mode):
    """
    For each image:
      - Rank patches by error (high → low)
      - Select k patches to REMOVE based on mode:
          'high'   → remove top-k highest error patches
          'low'    → remove top-k lowest error patches
          'random' → remove k random patches
      - Encode remaining patches
      - Mean-pool to get image representation
    ...
    for start in range(0, N, BSIZE):
        ...
        with torch.no_grad():
            for i in range(B):
                ...
                if mode == 'high':
                    ranked   = np.argsort(err_i)[::-1]   # highest first
                    remove   = set(ranked[:k].tolist())
                elif mode == 'low':
                    ranked   = np.argsort(err_i)          # lowest first
                    remove   = set(ranked[:k].tolist())
                else:  # random
                ...
                # Encode only kept patches
                img_batch = imgs[i].unsqueeze(0)           # (1, 3, 32, 32)
                tokens    = encoder(img_batch, patch_indices=keep_idx)  # (1, n_keep, D)
    ...
    print()
    return np.stack(reprs)   # (N, EMBED_DIM)


# ─────────────────────────────────────────────────────────────────────────────
# STEP 3: Linear probe under each ablation condition
# ─────────────────────────────────────────────────────────────────────────────
...
# Full representation — no ablation (sanity check)
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 4: Run experiment across all k values
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "="*60)
...
# ── Plot 1: Accuracy under ablation ──────────────────────────────────────────
ax = axes[0]
...
# STEP 6: Verdict
# ─────────────────────────────────────────────────────────────────────────────
# At largest k, is high-error drop > random drop?
max_k_idx     = -1
